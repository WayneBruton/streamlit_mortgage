...
loan_amont = home_value - deposit
monthly_interest_rate = interest_rate / 100 / 12
number_of_payments = loan_term * 12
monthly_payment = loan_amont * (monthly_interest_rate * math.pow(1 + monthly_interest_rate, number_of_payments)) / (math.pow(1 + monthly_interest_rate, number_of_payments) - 1)

total_payments = monthly_payment * number_of_payments
total_interest = total_payments - loan_amont
...
df = pd.DataFrame(schedule, columns=['Payment Number', 'Payment Amount', 'Principal', 'Interest', 'Remaining Balance', 'Year'])
